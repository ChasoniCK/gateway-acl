...
- **Управлять туннелями из Настроек.** Можно добавлять и удалять сколько угодно
  ...
  неудачном запуске панель возвращает прежний backend.
...
- Для управляемых туннелей заранее установите нужный инструмент: `sing-box`,
  `wg-quick` или `awg-quick`. Установщик их не ставит. Обычный NAT или другой
  уже настроенный маршрут работает и без них.
...
Туннели настраиваются в **Настройки → Туннели**. Установщик не ставит VPN-программы,
не скачивает подписки, не переписывает конфиги и не перезапускает туннели — в том
числе при обновлении с `--yes`.
...
только потом открывает трафик. При ошибке возвращаются прежние конфиг, служба и
метка; если не удался сам откат, транзит остаётся закрыт. Самостоятельное падение
туннеля замечается на следующем опросе панели, не мгновенно.
...
- Для управляемого quick-профиля нужен полный маршрут IPv4 и заранее
  установленная подходящая программа. Панель не ставит `sing-box`, WireGuard или
  AmneziaWG, не поддерживает хуки и собственные таблицы маршрутизации из
  профиля и отказывается переключаться, пока найден чужой туннель с default route.
