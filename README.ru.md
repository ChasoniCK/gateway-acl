...
- **...или выпустить мимо VPN, а не выключать.** Вторая кнопка уводит одно
  ...
  считает не своей: `"vpn_mark"` в `config.json`, `8228` (`0x2024`) для sing-box,
  `51820` для стандартного wg-quick, `0` — кнопки нет. Число берите со своего
  шлюза: соседняя метка делает ровно обратное. Метка ставится по MAC, поэтому
...
Дадите ссылку подписки — установщик поднимет и туннель. Ссылка ложится в
`/etc/gateway-acl/sub.url` (права 0600) и подставляется при следующем запуске,
так что обновление — это один Enter. Оставите ответ пустым — sing-box не
трогается вовсе.

Второй вопрос — какие узлы выкинуть: регулярное выражение по именам, которые дал
им провайдер, лежит в `/etc/gateway-acl/sub.exclude`. Значит он больше, чем
кажется: группа `proxy` — это `urltest`, она выбирает по задержке, а узел в вашей
же стране всегда быстрее прочих — и подписка, где такой есть, выпускает наружу
все устройства дома, ровно от чего туннель и заводили. Один `-` снимает фильтр.
...
Пишет он подчёркнуто мало: outbound'ы с тегом `sub-*` и состав группы `proxy`.
...
туннель, приватные адреса уходят через `direct`. Недоступный сервер подписки не
может сорвать установку: шаг предупредит, и панель поставится дальше.
