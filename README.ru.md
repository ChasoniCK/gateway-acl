...
сохранённому профилю, панель говорит об этом красным над списком: это его
...
`amneziawg-tools` нет в собственных репозиториях дистрибутивов: у Ubuntu он в PPA
проекта, у Arch — в AUR. Установщик предложит и то и другое, но только когда вы
за клавиатурой: сторонний источник пакетов и сборка чужого PKGBUILD — не то, что
решают за человека при обновлении с `--yes`. Сборка из AUR идёт от имени того,
кто набрал `sudo` (`makepkg` от root не работает) и требует `paru` или `yay`.
Если база pacman в этот момент заперта — идёт другое обновление или замок
остался от убитого `pacman`, — установщик ждёт минуту и говорит, что именно
из двух случилось, вместо того чтобы висеть на `:: В данный момент уже
работает Pacman`.
...
которому нечем поднять интерфейс. Такое состояние теперь и называется своим
именем: ни установщик, ни панель не считают одни только tools готовностью —
`awg-quick up` на таком хосте печатает `Unknown device type` и останавливается.

Ставятся `amneziawg-tools` и `amneziawg-go`, а модуль ядра `amneziawg-dkms` —
только если в системе есть заголовки текущего ядра, и его неудача ни на что не
влияет: `awg-quick` сам берёт модуль, когда он есть, и реализацию в пространстве
пользователя, когда его нет. На дистрибутиве со своим ядром это разница между
«работает» и «пересобери после каждого обновления ядра»; цена — скорость, и
установщик говорит, какой из двух режимов получился. Если поставить не удалось
вовсе, он пишет, какая это система.
...
Подписку можно **раскрыть** — кнопка «узлы» — и отметить, к каким её серверам
...
по одному и без угадывания шаблона. Рядом с каждым узлом стоит отметка от
